"""Tests of the Gtransfo objects and its helpers."""
import numpy as np
...
class GtransfoPolyBase(object):
    def setUp(self):
        ...
        self.polyIdentity = lsst.jointcal.gtransfo.GtransfoPoly(9)

        self.poly2 = lsst.jointcal.gtransfo.GtransfoPoly(2)
        ...
        self.poly3 = lsst.jointcal.gtransfo.GtransfoPoly(3)
        ...
        self.poly9 = lsst.jointcal.gtransfo.GtransfoPoly(9)
        # A "reasonable" polynomial from fitting some data.
...
class InversePolyTransfoTestCase(GtransfoPolyBase, lsst.utils.tests.TestCase):
    def checkInverse(self, poly, inverse, maxDiff):
        """Test that ``gtransfoPoly(inverse(point))==point`` to within maxDiff."""
        results = []
    ...
    def testInversePolyIdentity(self):
        precision = 1e-8
        inverse = lsst.jointcal.gtransfo.inversePolyTransfo(self.polyIdentity, self.frame, precision)
        self.checkInverse(self.polyIdentity, inverse, precision)

    def testInversePoly2(self):
        precision = 1e-6
        inverse = lsst.jointcal.gtransfo.inversePolyTransfo(self.poly2, self.frame, precision)
    ...
    def testInversePoly3(self):
        ...
        precision = 1e-7
        inverse = lsst.jointcal.gtransfo.inversePolyTransfo(self.poly3, self.frame, precision, maxDegree=5)
        self.checkInverse(self.poly3, inverse, 3e-8)

    def testInversePoly9(self):
        precision = 1e-6
        inverse = lsst.jointcal.gtransfo.inversePolyTransfo(self.poly9, self.frame, precision,
                                                            maxDegree=11, nSteps=100)
        self.checkInverse(self.poly9, inverse, 4e-5)

    def testNotEnoughPoints(self):
        with self.assertRaises(RuntimeError):
            lsst.jointcal.gtransfo.inversePolyTransfo(self.poly2, self.frame, 1e-4, nSteps=2)


class GtransfoPolyTestCase(GtransfoPolyBase, lsst.utils.tests.TestCase):
    def checkToAstMap(self, poly, inverseMaxDiff=1e-6):
        """Test that GtransfoPoly.toAstMap() gives accurate results.

        Parameters
        ----------
        poly : `lsst.jointcal.gtransfo.GtransfoPoly`
    ...
    def testToAstMapDegree2(self):
        self.checkToAstMap(self.poly2)

    def testToAstMapDegree3(self):
    ...
    def testToAstMapDegree9(self):
        # looser tolerance: 9th degree polynomials are harder to get a good inverse for.
