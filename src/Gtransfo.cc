...
std::unique_ptr<Gtransfo> Gtransfo::ReduceCompo(
        const Gtransfo *Right) const {  // by default no way to compose
    if (Right) {
    }  // avoid a warning
...
void Gtransfo::Derivative(const Point &Where, GtransfoLin &Der, const double Step) const {
    double x = Where.x;
    double y = Where.y;
    double xp0, yp0;
    apply(x, y, xp0, yp0);
...
void Gtransfo::ParamDerivatives(const Point &Where, double *Dx, double *Dy) const {
    if ((Where.x || Dx) || Dy) {
    }  // compilation warning killer
    throw LSST_EXCEPT(pex::exceptions::InvalidParameterError,
...
public:
    ...
    std::unique_ptr<Gtransfo> RoughInverse(const Frame &) const { return direct->Clone(); }

    //! Inverse transfo: returns the direct one!
    std::unique_ptr<Gtransfo> InverseTransfo(const double, const Frame &) const { return direct->Clone(); }
...
void GtransfoInverse::dump(ostream &stream) const {
    stream << " GtransfoInverse of  :" << endl << *direct << endl;
}
...
void GtransfoIdentity::Derivative(const Point &, GtransfoLin &Derivative, const double) const {
...
void GtransfoIdentity::Write(ostream &s) const { s << "GtransfoIdentity 1" << endl; }
...
//#ifdef TO_BE_FIXED
GtransfoPoly::GtransfoPoly(const Gtransfo *T, const Frame &F, unsigned Degree, unsigned NPoint) {
    StarMatchList sm;
...
void GtransfoPoly::compute_monomials(double Xin, double Yin, double *Monom) const {
...
void GtransfoPoly::Derivative(const Point &Where, GtransfoLin &Der, const double Step)
...
double GtransfoPoly::CoeffOrZero(const unsigned Degx, const unsigned Degy, const unsigned WhichCoord) const {
    //  assert((Degx+Degy<=deg) && WhichCoord<2);
    assert(WhichCoord < 2);
    if (Degx + Degy <= deg) return coeffs[(Degx + Degy) * (Degx + Degy + 1) / 2 + Degy + WhichCoord * nterms];
    return 0;
}
...
static const CoeffTagStruct CoeffTags[] = {
        {"dx", 0, 0, 0},   {"dy", 0, 0, 1},   {"a11", 1, 0, 0},   {"a12", 0, 1, 0},   {"a22", 0, 1, 1},
        {"a21", 1, 0, 1},  {"a1x2", 2, 0, 0}, {"a1xy", 1, 1, 0},  {"a1y2", 0, 2, 0},  {"a2y2", 0, 2, 1},
        {"a2xy", 1, 1, 1}, {"a2x2", 2, 0, 1}, {"a1x3", 3, 0, 0},  {"a1x2y", 2, 1, 0}, {"a1xy2", 1, 2, 0},
        {"a1y3", 0, 3, 0}, {"a2y3", 0, 3, 1}, {"a2xy2", 1, 2, 1}, {"a2x2y", 2, 1, 1}, {"a2x3", 3, 0, 1}};
...
static unsigned tag_pos(const char *Name) {
    for (unsigned k = 0; k < NCoeffTags; ++k) {
        ...
        if (strcmp(t.name, Name) == 0) return k;
    }
...
void GtransfoPoly::dump(ostream &S) const {
    ...
    if (deg > 0) S << " Linear Determinant = " << Determinant() << endl;
}

double GtransfoPoly::Determinant() const {
    if (deg >= 1) return Coeff(1, 0, 0) * Coeff(0, 1, 1) - Coeff(0, 1, 0) * Coeff(1, 0, 1);
    return 0;
}
...
double GtransfoPoly::fit(const StarMatchList &List) {
    if (List.size() < nterms) {
        LOGLS_FATAL(_log, "GtransfoPoly::fit trying to fit a polynomial transfo of degree "
                                  << deg << " with only " << List.size() << " matches.");
        return -1;
    }
...
public:
    ...
    PolyXY(const GtransfoPoly &P, const unsigned WhichCoord)
        ...
        for (unsigned px = 0; px <= deg; ++px)
            for (unsigned py = 0; py <= deg - px; ++py) Coeff(px, py) = P.Coeff(px, py, WhichCoord);
    }
...
static PolyXY operator*(const long double &a, const PolyXY &P) {
    ...
    for (unsigned i = 0; i <= deg; ++i)
        for (unsigned j = 0; j <= deg - i; ++j) result.Coeff(i, j) *= a;
    return result;
}
...
void GtransfoPoly::Write(ostream &s) const {
    ...
    s << setprecision(12);
    for (unsigned k = 0; k < 2 * nterms; ++k) s << coeffs[k] << ' ';
    s << endl;
...
std::unique_ptr<GtransfoPoly> InversePolyTransfo(const Gtransfo &Direct, const Frame &F, const double Prec) {
    ...
    for (degree = 1; degree <= maxdeg; ++degree) {
        ...
        double chi2 = 0;
        for (auto const &i : sm) chi2 += i.point2.Dist2(poly->apply((i.point1)));
        if (chi2 / npairs < Prec * Prec) break;
    }
    if (degree > maxdeg)
        LOGLS_WARN(_log,
                   "InversePolyTransfo: Reached max degree without reaching requested precision: " << Prec);
    return poly;
}
...
Point BaseTanWcs::TangentPoint() const { return Point(rad2deg(ra0), rad2deg(dec0)); }

GtransfoLin BaseTanWcs::LinPart() const { return linPix2Tan; }
...
void TanPix2RaDec::Pix2TP(double Xin, double Yin, double &Xtp, double &Ytp) const {
...
void TanSipPix2RaDec::Pix2TP(double Xin, double Yin, double &Xtp,
                             double &Ytp) const {  // Xtp, Ytp returned in degrees
    if (corr) {
        double xtmp, ytmp;
        corr->apply(Xin, Yin, xtmp, ytmp);
        linPix2Tan.apply(xtmp, ytmp, Xtp, Ytp);
    } else
        linPix2Tan.apply(Xin, Yin, Xtp, Ytp);
}
...
Point TanRaDec2Pix::TangentPoint() const { return Point(rad2deg(ra0), rad2deg(dec0)); }

GtransfoLin TanRaDec2Pix::LinPart() const { return linTan2Pix; }
...
TanPix2RaDec TanRaDec2Pix::invert() const { return TanPix2RaDec(LinPart().invert(), TangentPoint()); }
...
UserTransfo::UserTransfo(GtransfoFun &Fun, const void *UserData) : userFun(Fun), userData(UserData) {}
...
void UserTransfo::dump(ostream &stream) const {
    stream << "UserTransfo with user function @ " << userFun << "and userData@ " << userData << endl;
}
