...
class SimAstromCoaddTask(MakeCoaddTempExpTask):
    def getCalExp(self, dataRef, bgSubtracted):
        ...
        if not self.config.doApplyUberCal:
            return exposure
        # if we are here, it means that we have to apply the improved calibrations coming from meas_simAstrom
        self.log.info("doApplyUberCal is set - Using simAstrom updated calibrations")
        self.applySimAstromResults(dataRef, calexp=exposure)
        return exposure
    
    def applySimAstromResults(self, dataRef, calexp=None):
        ...
        """
        return self.applySimAstromResultsExposure(dataRef, calexp).exposure
        
    def applySimAstromResultsExposure(self, dataRef, calexp=None):
        """Update an Exposure with the Wcs, from meas_simAstrom 
