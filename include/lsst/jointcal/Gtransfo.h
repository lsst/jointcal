...
public:
    ...
    virtual void Derivative(const Point &Where, GtransfoLin &Der, const double Step = 0.01) const;
...
public:
    ...
    void Derivative(const Point &Where, GtransfoLin &Derivative, const double Step = 0.01) const;
...
private:
    ...
    void compute_monomials(double Xin, double Yin, double *Monom) const;
...
public:
    ...
    GtransfoPoly(const Gtransfo *T, const Frame &F, unsigned Degree, unsigned NPoint = 1000);
...
private:
    double do_the_fit(const StarMatchList &List, const Gtransfo &InTransfo, const bool UseErrors);
};
...
public:
    ...
    const GtransfoPoly *Corr() const { return corr.get(); }
