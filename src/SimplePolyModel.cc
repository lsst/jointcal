...
#ifdef SAVE
const TanSipPix2RaDec& SimplePolyModel::ProduceSipWcs(CcdImagePtr CcdPtr) const
{
  CcdImage &Ccd = *CcdPtr;  
...
//  if (!(&pix2Tp)  || ! proj) return NULL;
  ...
  return TanSipPix2RaDec(cdStuff, tangentPoint, &sip);
}

PTR(TanSipPix2RaDec) SimplePolyModel::ProduceSipWcs(CcdImagePtr CcdPtr) const
{
  CcdImage &Ccd = *CcdPtr;  
  const GtransfoPoly &pix2Tp=dynamic_cast<const GtransfoPoly&>(GetTransfo(Ccd));
  const TanRaDec2Pix *proj=dynamic_cast<const TanRaDec2Pix*>(Sky2TP(Ccd));
