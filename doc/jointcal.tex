...
The routines in the \ClName{AstrometricFit} class do not really evaluate
...
of which has to be provided to the \ClName{AstrometricFit} constructor.
...
errors through the fitted mapping. This introduces a dependency of the
$\chi^2$ on the parameters that we have decided not to track, because
...
propagate errors, even if mappings are updated.  

The matrix $W_j$ should be derived from
...
projector. The derivative of the reference residual $R^m_{j}$ with respect to
the \ClName{FittedStar} position $F_j$ (see eq. \ref{eq:chi2}),
requires to know explicitly the $2\times 2$ matrix of the derivative
...
the \ClName{AstrometricFit} class.  The routines
\RoutineName{AstrometricFit::LSDerivatives1} and
\RoutineName{AstrometricFit::LSDerivatives2} evaluate the contributions to
...
the smallest possible subset of SuiteSparse into jointcal\_cholmod. 
...
memory. The \RoutineName{AstrometricFit::AssignIndices} takes care of
...
\RoutineName{AstrometricFit::OffsetParams}.
...
\RoutineName{AstrometricFit::AssignIndices} takes a string argument that specifies
...
# Send back fitted stars on the sky.
        assoc.DeprojectFittedStars() # required for AstrometricFit
