...
\section{Proper motions and atmospheric refraction}
The expression \ref{eq:chi2} above depends on two sets of parameters:
...
\RoutineName{AstromFit::LSDerivatives2} evaluate the contributions of
the measurement terms and the references terms respectively to the
...
there is hence not much to ge gained in speeding up the calculation of
derivatives. For the factorization, we have tried both Eigen and Cholmod
(via the Eigen interface) and they are as fast within less than 10 \%. 
...
memory.  The \RoutineName{AstromFit::SetWhatToFit} takes care of
